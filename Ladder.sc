...
object Solution {
  def solution(a: Array[Int], b: Array[Int]): Array[Int] = {
    def fibs(limit: Int, mod: Int): Array[Int] = {
      val f = Array.ofDim[Int](limit + 2)
      f(0) = 0
      f(1) = 1
      for (i <- 2 until limit + 2) f(i) = (f(i - 1) + f(i - 2)) & mod
      f
    }

    val fib = fibs(a.length, (1 << b.max) - 1)
    a.indices.map(i => fib(a(i) + 1) & ((1 << b(i)) - 1)).toArray
  }
}
